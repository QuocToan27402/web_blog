...
router.post('/ResetPassword/:token', validator.validatorPassword() ,async function (req, res, next) {
...
router.post('/ForgotPassword', validator.validatorEmail(), async function (req, res, next) {
  var result = validationResult(req);
  console.log(result);
  if (result.errors.length > 0) {
    ResHand(res, false, result.errors);
